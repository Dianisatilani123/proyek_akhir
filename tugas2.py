...
def export_report_to_pdf(data, gender_counts, education_counts, experience_counts, company_type_counts, company_size_counts, discipline_counts, last_new_job_counts, figures):
    ...
    # Menambahkan jumlah pelamar berdasarkan tipe perusahaan sebelumnya
    pdf.cell(200, 10, txt="Jumlah pelamar berdasarkan tipe perusahaan sebelumnya:", ln=True)
    ...
    # Menambahkan gambar visualisasi ke PDF
    for fig in figures:
        pdf.add_page()
        pdf.image(fig, x=10, y=20, w=180)  # Menyesuaikan gambar agar pas di halaman

    # Simpan PDF ke file
    pdf.output("Laporan_Analitik_dan_Keberagaman.pdf")

    # Tampilkan tautan unduhan di Streamlit
    with open("Laporan_Analitik_dan_Keberagaman.pdf", "rb") as pdf_file:
        pdf_data = pdf_file.read()

    b64_pdf = base64.b64encode(pdf_data).decode('utf-8')
    pdf_href = f'<a href="data:application/octet-stream;base64,{b64_pdf}" download="Laporan_Analitik_dan_Keberagaman.pdf">Unduh Laporan PDF</a>'
    st.markdown(pdf_href, unsafe_allow_html=True)

# Langkah 8: Membuat halaman sidebar untuk navigasi
def sidebar_navigation():
    add_custom_css()
    st.sidebar.title("Navigasi")
    selection = st.sidebar.radio("Pilih Halaman:", ["Home", "Prediksi", "Laporan Keanekaragaman", "Upload Dataset"])
    return selection

def main():
    selection = sidebar_navigation()
    if selection == "Home":
        st.title("Selamat datang di Aplikasi Prediksi dan Laporan Keberagaman")
        st.write("Gunakan sidebar untuk menavigasi ke berbagai halaman dalam aplikasi ini.")
    ...
    elif selection == "Laporan Keanekaragaman":
        st.title("Halaman Laporan Keanekaragaman")
        data = load_data()
        data = preprocess_data(data)
        ...
        st.title("Halaman Upload Dataset")
